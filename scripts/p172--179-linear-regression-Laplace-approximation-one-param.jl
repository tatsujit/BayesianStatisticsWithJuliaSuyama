...
program_name = "p172--179-linear-regression-Laplace-approximation-one-param"
using CairoMakie
using LaTeXStrings
using Distributions
using Printf
...
################################################################
# linear regression with a single parameter
################################################################
...
################################################################
ulp(w1) = sum(logpdf.(Normal.(w1 * X_obs .+ w2, σ), Y_obs)) +
    logpdf(Normal(μ1, σ1), w1)
# dulp(w1) = ForwardDiff.derivative(ulp, w1)
...
lines!(ax12, w1s, ulp.(w1s))

ax13 = Axis(fig[1, 3],
            title = "unnormalized posterior",
            xlabel = L"w_1", ylabel = "density (unnormalized)")
lines!(ax13, w1s, exp.(ulp.(w1s)))

################################################################
# gradient method to find the maximum a posteriori (MAP)
################################################################
function gradient_method_1dim(f, x_init, μ, max_iter)
    df(x) = ForwardDiff.derivative(f, x)
    x_seq = Array{typeof(x_init), 1}(undef, max_iter)
    x_seq[1] = x_init
    for i in 2:max_iter
        x_seq[i] = x_seq[i-1] + μ * df(x_seq[i - 1])
        # maximum を探すから - ではなく +
    end
    return x_seq
end

function inference_wrapper_gd_1dim(log_joint, params, w1_init, μ, max_iter)
    ulp(w1) = log_joint(w1, params...)
    w1_seq = gradient_method_1dim(ulp, w1_init, μ, max_iter)
    return w1_seq
end

log_joint(w1, X, Y, w2, σ, μ1, σ1) =
    sum(logpdf.(Normal.(w1 * X .+ w2, σ), Y)) +
    logpdf(Normal(μ1, σ1), w1)
...
((w1_seq = gradient_method_1dim(ulp, w1_init, μ, max_iter)); nothing)
((w1_seq_wr = inference_wrapper_gd_1dim(log_joint, params, w1_init, μ, max_iter)); nothing)
...
if benchmark_flag
    ...
    res_wr = @benchmark begin
        w1_seq_wr = inference_wrapper_gd_1dim(log_joint, params, w1_init, μ, max_iter);
        nothing
    end
...
lines!(ax21, 1:max_iter, w1_seq)#, color = :blue)

if benchmark_flag
    text!(ax21, max_iter/3, 0.25,
          text = "time = $res_str ms",
          align = (:left, :bottom),
          color = :black)
    text!(ax21, max_iter/3, 0.15,
...
# data plot with MAP
w1s = range(-5, 5, length=500)
scatter!(ax12, [μ_approx], [ulp(μ_approx)], color = :red)
scatter!(ax13, [μ_approx], [exp(ulp(μ_approx))], color = :red)

grad(x) = ForwardDiff.derivative(ulp, x) # scalar
hessian(x) = ForwardDiff.derivative(grad, x) # scalar
σ_approx = sqrt(inv(-hessian(μ_approx)))

w1s_approx = range(-0.5, 1.0, length=100)
ax22 = Axis(fig[2, 2],
            title = "unnormalized posterior distribution",
            xlabel = L"w_1", ylabel = "density (unnormalized)")
ax23 = Axis(fig[2, 3],
            title = "approximate posterior (Laplace approximation)",
            xlabel = L"w_1", ylabel = "density")
lines!(ax22, w1s_approx, exp.(ulp.(w1s_approx)))
lines!(ax23, w1s_approx, pdf.(Normal(μ_approx, σ_approx), w1s_approx))
