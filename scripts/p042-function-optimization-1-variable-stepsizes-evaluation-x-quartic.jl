# cubic ならそりゃ最適化はできない。谷に落ちていく。そりゃそうか
################################################################
...
program_name = "p042-function-optimization-1-variable-stepsizes-evaluation-x-quartic"
...
Label(fig[0, 1:2],
      L"\text{Maximize }f(x)=%$(f_string)\text{ with various }ηs, \text{ }x_{\text{init}} = %$x_init",
...
for (i, η) in enumerate([0.01, 0.02, 0.03])
    xs = range(-200, 200, length=100)
    ax = Axis(fig[2, i-1],
              title = L"\eta = %$η", xlabel = L"x", ylabel = L"f(x)")
    x_seqs = data.x_seq[i+1]
    f_seqs = data.f_seq[i+1]
    lines!(ax, xs, f.(xs), color = :blue, label = "function")
    # lines!(ax, xs, fs, color = :blue, label = "function")
    scatter!(ax, x_seqs, f_seqs, color = :green, marker = :diamond, label = "optimization path")
    scatter!(ax, [x_opt], [f(x_opt)], color = :red, label = "optimal")
    # axislegend(ax; position = :rb)
end
