...
program_name = "p200--206-logistic-regression-MCMC"
using CairoMakie
using LaTeXStrings
using Distributions
using Printf
...
Label(fig[-1, 2:7],
      "logistic regression with MCMC (iteration = $(max_iter), burnin = $burnin)", #, (μ1, μ2, σ1, σ2) =  ($μ1, $μ2, $σ1, $σ2)",
...
"""
log_joint: ロジスティック回帰の対数同時確率

ForwardDiff との互換性のため：
- Bernoulli の p パラメータを [0, 1] にクリップ
- Dual 型でも安全に処理
"""
function log_joint(w, X, Y, μ1, σ1, μ2, σ2)
    # 事前分布
    ...
    # ロジスティック関数を通した予測確率
    p_raw = sig.(w[1] .* X .+ w[2])
    # 重要：Bernoulli が要求する [0, 1] 範囲にクリップ
    # これにより Dual 値でもエラーが出ない
    p_safe = clamp.(p_raw, 1e-10, 1.0 - 1e-10)
    # 尤度
    lp_likelihood = sum(logpdf.(Bernoulli.(p_safe), Y))
...
w1_limits = ((-20, 20), (0, 0.15)) # for rotated hist
w2_limits = ((-20, 20), (0, 0.15)) # for rotated hist
...
for i in 1:size(param_posterior_GMH, 2)
    ...
    fg(x) = sig(w1g*x+w2g);     fh(x) = sig(w1h*x+w2h)
    push!(fs_GMH, fg.(xs));     push!(fs_HMC, fh.(xs))
    lines!(ax32, xs, fg.(xs), color = (:green, 50/max_iter))
    lines!(ax35, xs, fh.(xs), color = (:green, 50/max_iter))
end
...
text!(ax334, -2.9, 0.75, text = "sigmoids-mean isn't a sigmoid", color = :purple)
text!(ax367, -2.9, 0.75, text = "sigmoids-mean isn't a sigmoid", color = :purple)
